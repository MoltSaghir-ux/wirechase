    ...
    <div className="flex min-h-screen bg-[#f8fafc]">
      ...
      <main className="flex-1 px-8 py-8 max-w-3xl">
        <div className="flex items-center justify-between mb-7">
          <div>
            <h2 className="text-2xl font-bold text-[#0f2240]">Document Templates</h2>
            <p className="text-gray-400 text-sm mt-0.5">Save checklist presets for quick client setup</p>
          </div>
          <button onClick={() => setShowNew(!showNew)}
            className="flex items-center gap-2 bg-[#c9a84c] text-[#0f2240] px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-[#a8893a] transition shadow-sm">
            + New Template
          </button>
        </div>
          ...
          <form onSubmit={saveTemplate} className="bg-white rounded-2xl border border-blue-100 shadow-sm p-6 mb-5 space-y-4">
                ...
                {MORTGAGE_PROGRAMS.map(p => (
                  <button key={p.id} type="button" onClick={() => setSelectedProgram(p.id)}
                    className={`text-left px-4 py-3 rounded-xl border text-sm transition ${
                      selectedProgram === p.id ? 'border-[#c9a84c] bg-[#fdf9ec] text-[#0f2240]' : 'border-gray-200 hover:border-gray-300 text-gray-700'
                    }`}>
            ...
            <div className="flex gap-3 pt-2">
              <button type="submit" disabled={saving || !name.trim() || !selectedProgram}
                className="bg-[#c9a84c] text-[#0f2240] px-6 py-2.5 rounded-xl text-sm font-semibold hover:bg-[#a8893a] disabled:opacity-40 transition">
