...
const STAGE_LABELS: Record<string, { label: string; color: string }> = {
  application: { label: 'Application', color: 'bg-gray-100 text-gray-600' },
  processing: { label: 'Processing', color: 'bg-blue-100 text-blue-700' },
  submitted_uw: { label: 'In UW', color: 'bg-purple-100 text-purple-700' },
  conditional_approval: { label: 'Cond. Approval', color: 'bg-yellow-100 text-yellow-700' },
  clear_to_close: { label: 'CTC', color: 'bg-emerald-100 text-emerald-700' },
  closing: { label: 'Closing', color: 'bg-teal-100 text-teal-700' },
      ...
      <main className="flex-1 px-8 py-8">
        ...
        {view === 'pipeline' ? (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            ...
            <PipelineView clients={filtered} loanByClient={loanByClient} />
          </div>
        ) : (
          /* Client list */
            ...
            {filtered.length > 0 ? (
              <div className="divide-y divide-gray-50">
                ...
                {filtered.map((client: DashboardClient) => {
                  ...
                  const stageInfo = STAGE_LABELS[stageKey] ?? { label: stageKey, color: 'bg-gray-100 text-gray-500' }
                  ...
                  return (
                    <Link key={client.id} href={`/broker/clients/${client.id}`} className="grid grid-cols-12 items-center px-6 py-4 hover:bg-gray-50/80 transition">
                      ...
                      <div className="col-span-2">
                        <span className={`inline-flex text-xs px-2 py-0.5 rounded-full font-medium ${stageInfo.color}`}>
                          {stageInfo.label}
                        </span>
                      </div>
              ...
              <div className="py-20 text-center">
                ...
                {!filter && !q && <Link href="/broker/loans/new" className="text-blue-600 text-sm font-medium hover:underline mt-2 inline-block">Add your first client →</Link>}
