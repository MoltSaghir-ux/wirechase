...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ filter?: string; q?: string; view?: string }> }) {
  ...
  const stats: { label: string; value: number; icon: ReactNode; color: string; filter: string | null }[] = [
    ...
    { label: 'Pending Docs', value: pending, icon: <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>, color: 'text-yellow-600 bg-yellow-50', filter: 'pending' },
