...
export async function POST(req: NextRequest) {
  ...
  const {
    clientName, approvedAmount, propertyAddress, expiryDate,
    conditions, loanOfficerName, loanOfficerNmls, brokerageName,
  ...
  const formattedAmount = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 0 }).format(approvedAmount)
...
<head>
  ...
  <title>Pre-Approval Letter — ${clientName}</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: 'Georgia', serif; color: #1a1a1a; background: #fff; }
    .page { max-width: 680px; margin: 0 auto; padding: 60px 60px; }
    .header { border-bottom: 3px solid #1e40af; padding-bottom: 24px; margin-bottom: 32px; display: flex; justify-content: space-between; align-items: flex-end; }
    .brand { font-family: 'Arial', sans-serif; }
    .brand-name { font-size: 28px; font-weight: 900; color: #1e40af; letter-spacing: -0.5px; }
    ...
    .badge { display: inline-block; background: #dbeafe; color: #1e40af; font-family: Arial, sans-serif; font-size: 11px; font-weight: 700; letter-spacing: 1px; text-transform: uppercase; padding: 6px 14px; border-radius: 4px; margin-bottom: 28px; }
    ...
    .amount-box { background: #f0f9ff; border: 2px solid #bae6fd; border-radius: 12px; padding: 20px 28px; margin: 28px 0; }
    .amount-label { font-family: Arial, sans-serif; font-size: 11px; font-weight: 700; color: #0284c7; letter-spacing: 1px; text-transform: uppercase; }
    .amount-value { font-size: 36px; font-weight: 900; color: #0c4a6e; margin-top: 4px; font-family: Arial, sans-serif; }
    ...
    .conditions { background: #fffbeb; border: 1px solid #fde68a; border-radius: 8px; padding: 16px 20px; margin: 24px 0; font-size: 13px; color: #78350f; line-height: 1.7; }
...
<body>
  <div class="page">
    <div class="header">
      <div class="brand">
        ...
        <div class="brand-sub">Mortgage Professionals</div>
      </div>
      <div class="date">${today}</div>
    </div>

    <div class="badge">Pre-Approval Letter</div>
    ...
    <p>
      This letter certifies that <strong>${clientName}</strong> has been pre-approved for a
      <strong>${loanTypeLabel} ${purposeLabel}</strong> mortgage loan${propertyAddress ? ` for the property located at <strong>${propertyAddress}</strong>` : ''}.
    </p>
    ...
    <p>
      This pre-approval is based on the information provided by the applicant and is subject to the
      conditions listed below. This letter does not constitute a commitment to lend.
    </p>
  ...
  const filename = `PreApproval_${clientName.replace(/\s+/g, '_')}_${new Date().toISOString().slice(0,10)}.html`
